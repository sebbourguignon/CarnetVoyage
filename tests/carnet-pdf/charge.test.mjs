...
test("charge de 80 photographies",{skip:process.env.CARNET_PDF_LOAD_TEST!=="1",timeout:180000},async(t)=>{
  ...
  const modele={version:1,voyage:{titre:"Grand voyage",dateDebut:"2026-07-01",dateFin:"2026-07-20"},statistiques:{journeesIllustrees:1,photos:80},journees:[{id:"jour",date:"2026-07-01",lieu:"Italie",titre:"Italie",introduction:"Paysages",recit:"Un récit de voyage.",tempsForts:[],distance:"",duree:"",carnetTerminee:true,photos}]};
  const resultat=await exporterCarnet(modele);
  try{
    assert.equal(resultat.diagnostic.pages,4);
