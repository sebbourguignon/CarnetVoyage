...
test("les légendes partagent le composant et le gap de référence Modène",()=>{const html=construireHtml({voyage:{titre:"Voyage"},statistiques:{photos:2},journees:[{id:"j",titre:"Jour",recit:"Récit",tempsForts:[],galeries:[],photos:[{dataUrl:"a",legende:"Légende"},{dataUrl:"b",legende:""}]}]});assert.match(html,/--carnet-photo-caption-gap:1\.5mm/);assert.match(html,/margin:var\(--carnet-photo-caption-gap\) 0 0/);assert.match(html,/class="carnet-photo"/);assert.match(html,/--carnet-photo-contain-space/);assert.match(html,/__carnetPhotosReady/);assert.equal((html.match(/<figcaption>/g)||[]).length,1);});
