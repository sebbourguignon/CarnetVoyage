-- Répare les environnements où 0025 a été marquée comme appliquée sans que
-- les colonnes de finalisation aient effectivement été créées.
alter table carnet_journees
  add column if not exists carnet_terminee boolean not null default false,
  add column if not exists recit_source_hash text;
